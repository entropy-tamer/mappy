...
use crate::benchmark::ml_tasks::{
    MLTaskResults, TagClassifier, TagClustering, TagEmbedding, TagSimilarity,
};
...
impl MLBenchmarkRunner {
    ...
    pub fn generate_ml_test_data(num_items: usize, tags_per_item: usize) -> Vec<Vec<String>> {
        let base_tags = vec![
            "anthro",
            "biped",
            "canid",
    ...
    pub async fn benchmark_similarity_search(
        ...
        for (idx, tags) in tag_sets.iter().enumerate() {
            ...
            mappy_maplet
                .insert(format!("item_{}", idx), compressed)
                .await?;
        }
    ...
    pub async fn benchmark_classification(
        ...
        let mut all_tags_for_corpus: Vec<String> = training_data
            .iter()
    ...
    #[cfg(feature = "mappy-integration")]
    pub async fn benchmark_clustering(tag_sets: &[Vec<String>], k: usize) -> Result<MLTaskResults> {
        // Exact clustering
        let start = Instant::now();
        ...
        for i in 0..tag_sets
            .len()
            .min(exact_clusters.len())
            .min(approximate_clusters.len())
        {
    ...
    pub async fn run_comprehensive_benchmarks() -> Result<Vec<MLTaskResults>> {
        ...
        let similarity_result =
            Self::benchmark_similarity_search(&tag_sets, &query_tags, 10).await?;
        ...
        let test_queries: Vec<Vec<String>> =
            test_data.iter().map(|(_, tags)| tags.clone()).collect();
        let expected_classes: Vec<String> =
            test_data.iter().map(|(class, _)| class.clone()).collect();

        println!("Running classification benchmark...");
        let classification_result =
            Self::benchmark_classification(&training_data, &test_queries, &expected_classes, 5)
                .await?;
