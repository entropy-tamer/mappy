...
async fn main() -> anyhow::Result<()> {
    ...
    for result in &results {
        ...
        println!("  Exact Accuracy:     {:.2}%", result.exact_accuracy * 100.0);
        println!("  Approximate Accuracy: {:.2}%", result.approximate_accuracy * 100.0);
        println!("  Accuracy Difference:  {:.4}%", result.accuracy_difference * 100.0);
        ...
        println!("  Acceptable:         {}", if is_acceptable { "✓ YES" } else { "✗ NO" });
        println!();
    }
    ...
    println!("Average Accuracy Difference: {:.4}%", avg_accuracy_diff * 100.0);
    println!("Average Speed Ratio:        {:.2}x", avg_speed_ratio);
    println!("All Tasks Acceptable:       {}", if all_acceptable { "✓ YES" } else { "✗ NO" });
    println!();
    ...
    let report_path = MLReportGenerator::generate_ml_report(&results, "reports/ml_benchmark_report.html")?;
